...
public class MomazonPrimeVideo extends StreamingCompany {
    ...
    /** El cobrador de Momazon. */
    private StrategyMomazon cobrador;
     ...
     */
    public MomazonPrimeVideo(String name, ArrayList<Service> services, Bank bank) {
    ...
    public void charge() {
        ...
        for (Client c : clients) {
            ...
            if (s.equals(services.get(0))) {
                cobrador = new MomazonNormalUser();
                boolean payment = cobrador.charge(c, 110, bank);
                notifyPayment(c, 110);
                if (!payment) toRemove.add(c);
            } else if (s.equals(services.get(1))) {
                cobrador = new MomazonPremiumUser();
                boolean payment = cobrador.charge(c, 150, bank);
                notifyPayment(c, 150);
                if (!payment) toRemove.add(c);
            }
        }
