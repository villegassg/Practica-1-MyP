...
public class Spootify extends StreamingCompany {
    ...
    /** El cobrador de Spootify. */
    private StrategySpootify cobrador;
     ...
     */
    public Spootify(String name, ArrayList<Service> services, Bank bank) {
    ...
    public void notifyRecommendation() {
        ...
        String recommendation = String.format("%s recommends you to listen to %s.", 
    ...
    public void charge() {
        ...
        for (Client c : clients) {
            ...
            if (s.equals(services.get(0))) {
                cobrador = new SpootifyNormalUser();
                boolean payment = cobrador.charge(c, 0, bank);
                notifyPayment(c, 0);
                if (!payment) toRemove.add(c);
            } else if (s.equals(services.get(1))) {
                cobrador = new SpootifyPremiumUser();
                boolean payment = cobrador.charge(c, 80, bank);
                notifyPayment(c, 80);
                if (!payment) toRemove.add(c);
            }
        }
